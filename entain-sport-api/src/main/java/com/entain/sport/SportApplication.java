...
public class SportApplication {
	...
	public CommandLineRunner demo(SportTypeRepository repository, SportEventRepository eventRepo) {
		return (args) -> {
			...
			eventRepo.save(new SportEvent("Eurobasket 2025", ldt.plus(Period.ofMonths(6)), EventStatus.INACTIVE,
					repository.save(new SportType("basketball"))));

			eventRepo.save(new SportEvent("World Cup 2026", LocalDateTime.parse("2026-06-11T12:00:00"),
					EventStatus.INACTIVE, repository.save(football)));

			eventRepo.save(new SportEvent("Old volleyball competition", ldt.minus(Period.ofDays(30)),
					EventStatus.INACTIVE, repository.save(new SportType("volleyball"))));
		};
	}
